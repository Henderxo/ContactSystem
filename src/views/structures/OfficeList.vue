...
onMounted(async () => {
      await getCollection({collectionName: 'Offices'});  
})
async function loadNewPage(newPage: Number){
    await getCollection({collectionName: 'Offices', page: newPage});
}
</script>

<template>
    <div style="height: 581px">
        <TableDisplay v-if="!isLoading" :data="(data as Office[])"></TableDisplay>
